...
- Run `dotnet test` after every change. Smoke tests must pass on `examples/REF/PPTX/Presentation1.pptx` and `pres-pro.pptx`.
...
- `examples/REF/PPTX/Presentation1.{pptx,pdf}` and `pres-pro.{pptx,pdf}` — PPTX smoke tests
