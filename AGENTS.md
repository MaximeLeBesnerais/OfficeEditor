...
- Run `dotnet test` after every change. Smoke tests must pass on `examples/REF/PPTX/northwind-demo.pptx` (self-made replacement REF deck; the former third-party REF decks were removed for licensing — replacements pending).
...
- `examples/REF/PPTX/`, `AetherLink-Glass-Shareholder-Overview`, `-Architecture-Overview`, `` — removed (licensing), replacements pending
- `examples/REF/DOCX/-entreprise-bcp-pme.{docx,pdf}` — DOCX regression; `Monitoring Report Template` and `Annual reporting template ENGLISH_0` removed (licensing), replacements pending
