...
- Run `dotnet test` after every change. Smoke tests must pass on `examples/REF/PPTX/northwind-demo.pptx` (self-made replacement REF deck; the former third-party REF decks were removed pre-public-release for licensing — replacements pending).
...
- `examples/REF/PPTX/pres-pro`, `AetherLink-Glass-Shareholder-Overview`, `FusionFest-Architecture-Overview`, `pitch-deck` — removed pre-public-release (licensing), replacements pending
- `examples/REF/DOCX/gestion-risques-entreprise-bcp-pme.{docx,pdf}` — DOCX regression; `Monitoring Report Template` and `Annual reporting template ENGLISH_0` removed pre-public-release (licensing), replacements pending
- Generated outputs: `examples/output/ref/`
- PNG mode: `--format png` generates per-slide images at the configured PPI
- Token sets: `PptxEditor.Core/Generation/Design/` — mined from REF decks (pres-pro, AetherLink)
