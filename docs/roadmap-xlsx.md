...
- **Sheet-name validation** — `AddWorksheet` accepts anything: duplicates, >31 chars, `: \ / ? * [ ]`. Loud validation with a domain exception (per AGENTS.md), checked against `_worksheets`. **S.** **Acceptance:** each invalid case throws a typed exception with a clear message.
...
- **README honesty** — root `README.md` tagline promises "instruction sets (JSON/YAML) … across all formats" and lists "charts" under Rich Content. Qualify both: instructions are DOCX/PPTX today; XLSX is fluent-API + variables only. **S.**
...
- **Multi-sheet stress tests** — 50+ sheets, cross-sheet formula webs, 100k-row sheet write/read round-trip within a time budget. **M.**
