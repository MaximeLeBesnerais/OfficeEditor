...
- **Column-name math past Z** — ~~suspected wrap-boundary bug~~: verified a non-issue. `GetColumnName` was exercised across the full 0–16383 range and found correct; it was replaced with the standard algorithm for clarity and boundary tests (ZZ, Z→AA, ZZ→AAA) were pinned. Done on the XLSX Phase 0 branch.
