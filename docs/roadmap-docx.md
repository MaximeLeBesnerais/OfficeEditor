...
- **Fix or replace `examples/Docx/instructions/sample.json`** — uses 4 nonexistent op types (`AddHeading`, `AddBulletList`, `Replace`, `AddTable`), and its one valid op (`AddParagraph`) uses field `content` where the parser requires `Text` — zero ops in the sample execute. Either rewrite it to the 4 working ops or (better) implement the missing ops in `InstructionEngine`. **Why:** first file a new user runs; currently broken. **S.** *Acceptance:* sample runs end-to-end via CLI; an integration test executes it.
- **Complete the instruction engine** — implement `AddRichContentInstruction` / `ReplaceWithRichContentInstruction` (currently fall into `NotSupportedException`) by delegating to the existing `DocumentBuilder.AddRichContent` / `ReplaceWithRichContent`. Teach `DocxJsonInstructionParser` the rich-content ops (it rejects them at parse time today) and add a loud schema validator for instruction JSON (unknown `type`, missing required fields → descriptive error, not silent skip). **S–M.** *Acceptance:* all 6 ops in `Models/Instructions.cs` dispatch; malformed JSON fails loudly with field-level messages.
- **Fix variable-merge formatting loss** — `DocxVariableReplacer.ReplaceVariablesAcrossRuns` collapses multi-run variables into the first run's `Text` and empties the rest, losing run-level formatting mid-paragraph; the orphaned `DocxTemplateEngine` (conditionals/loops only, no production callers) has the same single-`Run` rebuild (`paragraph.Append(new Run(new Text(text)))`). Replace runs in-place, preserving `RunProperties` of the first affected run. **Why:** silent data corruption in the flagship mail-merge scenario. **M.** *Acceptance:* bold/italic/mixed-format paragraphs survive `MergeVariables`; regression test with a multi-run template.
