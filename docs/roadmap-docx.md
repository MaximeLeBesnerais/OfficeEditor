...
- **Fix or replace `examples/Docx/instructions/sample.json`** — ~~uses 4 nonexistent op types (`AddHeading`, `AddBulletList`, `Replace`, `AddTable`), and its one valid op (`AddParagraph`) uses field `content` where the parser requires `Text` — zero ops in the sample execute~~. Rewritten to supported ops (`create`, `addParagraph`, `replaceText`, `insertAfter`) with root `version`/`description` metadata, and `InstructionEngineTests.SampleJson_ShouldParseAndExecuteEndToEnd` parses and executes it end-to-end. **Done on the `night/docx-hardening` branch.**
- **Complete the instruction engine** — ~~implement `AddRichContentInstruction` / `ReplaceWithRichContentInstruction` (currently fall into `NotSupportedException`)~~. All 6 ops in `Models/Instructions.cs` dispatch (`AddRichContent`/`ReplaceWithRichContent` delegate to the builder); the JSON and YAML parsers accept the rich-content ops, and `DocxInstructionValidator` reports field-level errors (unknown/missing `type`, wrong value kinds). `InstructionEngine.Execute` now **prevalidates every operation before the first mutation**, so a malformed batch fails atomically instead of partially applying earlier ops. **Done on the `night/docx-hardening` branch.**
- **Fix variable-merge formatting loss** — ~~`DocxVariableReplacer.ReplaceVariablesAcrossRuns` collapses multi-run variables into the first run's `Text` and empties the rest, losing run-level formatting mid-paragraph~~. The cross-run replacer now rewrites the placeholder portion in-place within each affected `Text` node, preserving every run's `RunProperties`; suffix whitespace is kept via `xml:space="preserve"`, and a replacement value containing `{{...}}` is not re-substituted. Bold/italic/mixed-format paragraphs survive `MergeVariables`/`MergeBatch` (regression-tested in `VariableBranchTests`). The orphaned `DocxTemplateEngine` (conditionals/loops only, no production callers) still has the single-`Run` rebuild pattern and remains deferred. **Done on the `night/docx-hardening` branch.**
- **Create the numbering part for lists** — ~~`ContentBlockRenderer` hardcodes `NumberingId.Val = 1/2` without creating a `NumberingDefinitionsPart`, producing lists that only render if the template already defines those IDs~~. Rich-content bullet/ordered lists now create a `NumberingDefinitionsPart` on demand; ids are allocated above the host document's numbering (collision-free), and one generated abstract definition per kind (bullet / ordered) is reused across same-kind lists — the host document's existing numbering is never touched. Default single-level bullets and decimal numbering only. **Done on the `night/docx-hardening` branch.** *Remaining:* user-definable multi-level/custom schemes stay in Phase 1.7.
- **Harden the open/parser/merge boundary** — `DocumentBuilder.Create`/`Open` reject null/empty/whitespace paths, clean up partially-opened packages on failure (no leaked handles), and normalize only the known malformed-package failure types (`OpenXmlPackageException`, `FileFormatException`, `InvalidDataException`, `XmlException`) to the DOCX domain exception — ordinary path/permission and IO errors still propagate unchanged. The JSON/YAML parsers reject null or malformed input loudly (the JSON parser also rejects unknown fields); `InstructionEngine.Execute` prevalidates the whole batch before the first mutation; `MergeBatch` validates all records up front and writes each record to a temp file in the destination directory before atomically moving it into place, so a failed record leaves no partial output while already-completed records survive. **Done on the `night/docx-hardening` branch.**
- **Restore DOCX visual-regression outputs** — `examples/output/ref/` has `pptx/` but no `docx/`; the docx suite is wired in `tools/visual-diff` (`SuiteCatalog`) but has no reference outputs. Generate reference outputs for the license-clean DOCX corpus (`examples/REF/DOCX/annual-report.docx`, `examples/REF/DOCX/monitoring-report.docx`) and run them through the existing suite. **M.** *Acceptance:* docx RMSE check runs in CI alongside the pptx suite.
- **README/example honesty pass** — `examples/Docx/README.md` and NuGet READMEs must state what works today (no images, no headers/footers via builder, instruction set = 4–6 edit ops). **S.** *Acceptance:* no documented feature throws `NotSupportedException`. **Done:** the example README states the working set and its limitations and lists all 6 edit ops.
...
`MarkdownParser` + `AddMarkdown` already do md → docx. This phase closes the loop. **It does not depend on Phase 1:** the exporter *reads*, and the reading muscle already exists — the `DocxToTypstConverter` parses images, lists, tables, headers/footers today (hyperlinks are currently flattened to plain text; parsing them is new work, folded into Phase 1.3). The exporter consumes the converter's block walk, not the builder's write API, so it can run in parallel with Phase 1.
