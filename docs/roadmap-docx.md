...
- **Restore DOCX visual-regression outputs** — `examples/output/ref/` has `pptx/` but no `docx/`; the visual-diff suite is a stub for DOCX. Generate reference outputs for `examples/REF/DOCX/gestion-risques-entreprise-bcp-pme.docx` and wire them into `tools/visual-diff`. **M.** *Acceptance:* docx RMSE check runs in CI alongside the pptx suite.
