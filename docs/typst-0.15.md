...
- Action: **watch/verify** — after the crate bump, regenerate the parity fixtures and re-run visual-diff on `examples/REF/PPTX/pres-pro.pptx` and `AetherLink-...pptx` before claiming the upgrade done. **Adopt (later)**: convergence diagnostics improve our error surfacing in `TypstCompilerService`.
