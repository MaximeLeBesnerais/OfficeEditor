...
- Action: **watch/verify** — after the crate bump, regenerate the parity fixtures and re-run visual-diff on `examples/REF/PPTX/northwind-demo.pptx` before claiming the upgrade done (pres-pro / AetherLink REF decks removed pre-public-release for licensing). **Adopt (later)**: convergence diagnostics improve our error surfacing in `TypstCompilerService`.
