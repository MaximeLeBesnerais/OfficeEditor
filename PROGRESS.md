...
| 0.3 — Fix variable-merge formatting loss | DONE | _(pending)_ | Replaced in-place run rebuild in DocxVariableReplacer.ReplaceVariablesAcrossRuns; preserves RunProperties of affected runs; DocxTemplateEngine has same single-Run rebuild pattern but is orphaned (no production callers) — noted, not fixed |
