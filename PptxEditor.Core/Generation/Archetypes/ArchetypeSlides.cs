...
public static class ArchetypeSlides
{
    ...
    public static ContainerElement Cover(CoverContent content, DesignTokens design)
    {
        ...
        [
            TitleBlock(content.Kicker, content.Title, content.Subtitle, size: HeadingSize(content.Kicker, content.Subtitle, metrics)),
            AccentDivider(width: 2)
        ]);
    }
    ...
    public static ContainerElement Section(SectionContent content, DesignTokens design)
    {
        ...
        var metrics = design.Metrics;
        var children = new List<GenElement> { AccentDivider(width: 2) };
        if (content.Index is not null)
        {
            ...
            children.Add(new ContainerElement
            {
                ...
                Children =
                [
                    Kpi(content.Index, content.Kicker ?? "SECTION", delta: null, size: new SizeSpec { Grow = 1 }),
                    TitleBlock(kicker: null, content.Title, content.Subtitle, size: new SizeSpec { Grow = 2 })
                ]
            });
        }
        else
        {
            children.Add(TitleBlock(content.Kicker, content.Title, content.Subtitle, size: HeadingSize(content.Kicker, content.Subtitle, metrics)));
        }
        return Slide(design, EdgeInsets.All(metrics.MarginPt), Justify.End, metrics.GutterPt, children);
    }
    ...
    public static ContainerElement KpiRow(KpiRowContent content, DesignTokens design)
    {
        ...
        {
            children.Add(TitleBlock(kicker: null, content.Title, content.Subtitle, size: HeadingSize(null, content.Subtitle, metrics)));
        }
        children.Add(new ContainerElement
        {
            ...
            Children = content.Kpis
                .Select(kpi => Kpi(kpi.Value, kpi.Label, kpi.Delta, size: new SizeSpec { Grow = 1 }))
                .ToList<GenElement>()
        });
        return Slide(design, EdgeInsets.All(metrics.MarginPt), Justify.Start, metrics.GutterPt, children);
    }
    ...
    public static ContainerElement TwoCol(TwoColContent content, DesignTokens design)
    {
        ...
        children.Add(new ContainerElement
        {
            ...
            Children =
            [
                Slot(content.Left, weights?[0] ?? 1, "two_col", "left"),
                Slot(content.Right, weights?[1] ?? 1, "two_col", "right")
            ]
        });
        return Slide(design, EdgeInsets.All(metrics.MarginPt), Justify.Start, metrics.GutterPt, children);
    }
    ...
    private static ContainerElement Slide(DesignTokens design, EdgeInsets padding, Justify justify, double gap, IReadOnlyList<GenElement> children)
        => new()
        {
    ...
    private static ComponentElement TitleBlock(string? kicker, string title, string? subtitle, SizeSpec? size = null)
        => new() { Name = "title_block", Size = size, Content = ContentOf(new { kicker, title, subtitle }) };
    ...
    private static ComponentElement Kpi(string value, string label, string? delta, SizeSpec? size = null)
        => new() { Name = "kpi", Size = size, Content = ContentOf(new { value, label, delta }) };

    private static ComponentElement AccentDivider(double width)
        => new() { Name = "divider", Content = ContentOf(new { color = "accent", width }) };

    /// <summary>Assigns the slot's grow weight; the archetype owns slot geometry, never the author.</summary>
    private static ComponentElement Slot(ComponentElement slot, double grow, string archetype, string slotName)
    {
