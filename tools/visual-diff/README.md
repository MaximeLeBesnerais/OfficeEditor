...
# PPTX suite: examples/REF/PPTX/{Presentation1,pres-pro}.pdf vs examples/output/ref/pptx/*.pdf
...
`--suite pptx` diffs the two committed reference PDFs (`examples/REF/PPTX/Presentation1.pdf`, `pres-pro.pdf`) against generated PDFs under `examples/output/ref/pptx/`. Generated PDFs are produced by the repo's existing converter, `tools/convert-pptx`. If one is missing, the suite prints the exact command and skips that deck:
