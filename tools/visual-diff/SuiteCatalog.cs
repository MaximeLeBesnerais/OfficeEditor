...
{
    // NOTE: the  /  / Annual reporting / Monitoring Report REF fixtures
    // were removed (licensing). Their paths stay below so the suites
    ...
    public static List<ComparisonInput> BuildInputs(CliOptions options, ToolPaths tools)
    {
        if (options.Suite is not null)
        {
            if (string.Equals(options.Suite, "docx", StringComparison.OrdinalIgnoreCase))
            {
                return
                [
                    new("-entreprise-bcp-pme", "examples/REF/DOCX/-entreprise-bcp-pme.pdf", "examples/output/ref/docx/-entreprise-bcp-pme.pdf", InputKind.Pdf)
                ];
            }
