...
var deckPaths = new[]
{
    Path.Combine(repoRoot, "examples", "REF", "PPTX", "Presentation1.pptx"),
...
static string FindRepoRoot()
{
    ...
    foreach (var start in new[] { AppContext.BaseDirectory, Directory.GetCurrentDirectory() })
    {
        ...
        {
            if (File.Exists(Path.Combine(directory.FullName, "examples", "REF", "PPTX", "Presentation1.pptx")))
            {
    ...
    throw new DirectoryNotFoundException(
        "Could not locate the repository root (no ancestor directory contains examples/REF/PPTX/Presentation1.pptx).");
}
