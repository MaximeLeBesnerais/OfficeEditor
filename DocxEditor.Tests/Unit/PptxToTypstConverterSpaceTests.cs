...
/// <summary>
/// Regression tests for defects found on the "Space" deck (showeet):
///
...
public sealed class PptxToTypstConverterSpaceTests : IDisposable
{
    ...
    // ------------------------------------------------------------------
    // Pie chart support (Space slide 15 rendered a blank placeholder box)
    // ------------------------------------------------------------------
    ...
    /// <summary>Synthetic pie chart part mirroring Space slide 15's chart1.xml.</summary>
    ...
    // ------------------------------------------------------------------
    // Hyperlink run color (Space slide 19's www.showeet.com renders in the
    ...
    public void Convert_HyperlinkRun_UsesThemeHlinkColor()
    {
        ...
        using (var document = PresentationDocument.Create(deckPath, PresentationDocumentType.Presentation))
        {
            ...
            var hyperlinkRel = slidePart.AddHyperlinkRelationship(
                new Uri("https://www.showeet.com/", UriKind.Absolute), true);
            var textBox = new P.Shape(
                new NonVisualShapeProperties(
                ...
                new P.TextBody(
                    ...
                    new Drawing.Paragraph(
                        new Drawing.Run(
                            ...
                            new Drawing.Text("www.showeet.com")))));
            slidePart.Slide = new Slide(new CommonSlideData(CreateShapeTree(textBox)));
            slidePart.AddPart(slideLayoutPart);
