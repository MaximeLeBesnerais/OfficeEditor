...
/// <summary>
/// Deck-fidelity batch 1 (branch fix/deck-batch1):
/// 1. a:grpFill — freeforms inside groups must inherit the parent group's fill
///    (FISHBONE slides 2/3/4/9, FRANCE map groups 23/40/115).
/// 2. p:style a:fillRef/a:lnRef — shapes whose fill/stroke come only from the theme
///    format scheme must not be dropped (Opposites slide 5, Groups 10/11).
/// 3. cap="small" — small-caps text must be emitted (FRANCE headers, master titleStyle).
/// </summary>
public class PptxToTypstConverterDeckB1Tests : IDisposable
{
    private const long EmusPerPoint = 12700;

    private readonly string _tempDir;

    public PptxToTypstConverterDeckB1Tests()
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "PptxDeckB1Tests", Guid.NewGuid().ToString("N"));
    ...
    {
        var path = CreateDeck("cap-small-run.pptx", TextShapeWithCap(2, "Map of France", "small"));
    ...
    {
        var path = CreateDeck("cap-all-run.pptx", TextShapeWithCap(2, "Map of France", "all"));
    ...
    {
        // FRANCE header pattern: slideMaster titleStyle defRPr cap="small", slide
        ...
        var path = CreateDeck("cap-small-master.pptx", withTheme: false, smallCapsTitleStyle: true,
            TitlePlaceholder(2, "Map of France"));
    ...
    /// the FISHBONE/FRANCE group-child pattern.
    /// </summary>
